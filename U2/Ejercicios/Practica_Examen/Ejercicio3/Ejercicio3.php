...
<body>
    <form action="" method="post">
        ...
        <div>
            <label>Pago</label><br/>
            <input type="radio" name="pago" value="Efectivo" <?php rellenarRadio('Efectivo',false);?>/>Efectivo
            <input type="radio" name="pago" value="Tarjeta" <?php rellenarRadio('Tarjeta',true);?>/>Tarjeta
        </div>
        <br/>
        <div>
            ...
            if(empty($_POST['nombre']) or empty($_POST['dni'])  or empty($_POST['numero'])){
                ...
                if(isset($_POST['pago']) and $_POST['pago']=='Efectivo' and $_POST['numero']>2){
                    ...
                    if(!isset($error)){
                        ...
                        echo '<h3 style="color:blue;">Entrada correcta. El importe de la estancia 
                        es de '.$importe.'</h3>';


                        $estancia = new Estancia($_POST['dni'],
                                                $_POST['nombre'],
                                                $_POST['tipoH'],
                                                $_POST['numero'],
                                                $_POST['estancia'],
                        ...
                        if($_POST['pago']=='Efectivo'){
                            $estancia->setPago1(true);
                        }
                        else{
                            $estancia->setPago2(true);
                        }

                        if(isset($_POST['opciones'])){
                            foreach($_POST['opciones'] as $i){
                                switch($i){
                                    case '1':
                                        $estancia->setOpcion1(true);
                                        break;
                                    case '2':
                                        $estancia->setOpcion2(true);
                                        break;
                                    case '3':
                                        $estancia->setOpcion3(true);
                                        break;
                                }
                            }
                        }

                        if($modelo->crearEstancia($estancia)){
                            echo '<h3 style="color:blue">Estancia creada</h3>';
                        } else{
                            echo '<h3 style="color:red">Estancia no creada</h3>';
                        }
                    }
